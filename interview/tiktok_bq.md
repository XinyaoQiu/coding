...
### Q9. 通知处理的演进（时序 + 并发锁的故事，按时间线讲）

> 这是 premium 主力故事讲"技术难点"时的核心叙事，用**演进式**讲——简单实现 → 遇到问题 → 加机制 → 又遇到问题 → 再加。展示思考过程，不是端出一个完美方案。entry 级用这个正好。踩坑 PR#3632 嵌在时序那步。CAS 不主动讲、被问才答（口径见末尾）。

我会把它讲成一个一步步演进的故事，因为这套东西不是一开始就想全的，是遇到一个问题解决一个。

**第一步，最简单的实现。** 收到 Apple/Google 的通知，就读出这个用户的订阅记录、根据通知算出新状态、写回去。先跑起来。

**第二步，发现并发问题。** 同一个用户，平台可能在很短时间内发来好几条通知——比如一条扣款失败、紧跟一条重试成功。如果它们同时进来，两个 worker 都读到同一份旧记录、各自算、各自写，后写的把先写的覆盖掉,这就是并发的 read-modify-write 覆盖。所以我加了一个针对单个用户的分布式锁,用 Redis 的 SetNX 实现,key 是用户 id——同一个用户的通知必须串行处理,不同用户之间不阻塞。这样同一用户的读-改-写就不会互相踩。

**第三步,发现乱序问题。** 就算串行了,通知的到达顺序也不等于事件真正发生的顺序——平台不保证顺序,还会延迟、重投。比如"续费成功"可能比更早的"扣款失败"先到。如果按到达顺序傻处理,就会用旧事件覆盖新状态。所以我在每条记录上存一个"最后处理的事件时间"当水位线,新通知只有比它更新才处理,旧的丢掉。**这里其实踩过一个坑**:一开始我比的是数据库记录的更新时间,但因为处理有延迟,这个更新时间永远晚于事件真正发生的时间,结果反而把新的通知当成旧的丢掉了。后来专门加了一个字段存**平台自己签名的事件时间**,拿这个比,而不是用我们服务器的写入时间——这样才屏蔽掉处理延迟和多机时钟的干扰。

**第四步,发现重复问题。** 平台会重投通知(比如我们一时没返回成功,它就重发)。串行 + 时序判断能挡住大部分重复——重投的那条时间不比水位线新,会被丢。但更稳的是做一层显式幂等。用平台通知自带的唯一 id(Apple 的 notificationUUID、Google 的 message id)当幂等键,处理前先占位:我放在 Redis,用 `SET idem:{uuid} NX EX`(加个几天的 TTL),这条命令原子地"没见过才占位",占位成功才处理、撞了就直接跳过。放 Redis 而不是数据库,是因为一条 SetNX 就够、还能自动过期,比在数据库建唯一索引轻;代价是 Redis 万一丢了可能漏一次,但状态写本身是幂等的(重复处理一条"续费到同一到期时间"结果一样),漏一次也没副作用。

**收尾**:所以这套是"锁管并发、事件时间水位线管乱序、UUID 幂等管重复",三个正交的机制,各挡一类"不可靠输入"带来的问题。真正的难点不在任何单个机制,而在于意识到"我们完全依赖一个乱序、重复、延迟的平台通知流,得让最终状态始终正确"——这是个没有单点解法的系统性问题。

**被问到"有没有更好的办法 / 为什么用锁"时才答(不主动讲)**:并发和乱序这两块,其实更干净的做法是把判断下沉到 mongo 的条件更新——`findOneAndUpdate` 带一个 `event_time < 我的` 的条件,正确性由数据库单文档原子性保证,就不需要锁了。我考虑过这个方向,当前是用锁实现的,在我们这个量级下工作得很好、没出过问题,所以没有立刻改;如果并发或规模上来了,这是我会优先演进的方向。(⚠️ 口径:用"当前量级够用、按需演进",不要用"代码写完了只能后续评估"那种弱理由,会被追"改个 filter 而已"。锁的 wrong-holder delete / Lua 那块不主动讲,极小概率被抠锁实现才提。)
