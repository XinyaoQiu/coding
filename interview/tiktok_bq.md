...
> ⚠️ 口径已更新（跟 Q9 一致）：**"最难"首选讲"乱序/重复通知的整体正确性"这个系统性难题**（见 Q9 演进故事，没有单点解法，HM 绕不出简单解）；**别把"并发锁"单拎出来当最难**——它有 mongo 条件更新（CAS）这个一行更优解，被追"为什么不用条件更新"会露。下面两段是备用材料：**状态机设计**（双字段，站得住、无更优解，可当第二难点）优先于**并发锁**（有 CAS 更优解，只在被追问并发时诚实带出、别主动当"最难"端出）。都用 we，机制代码核实过。别说没核实的影响用户数。

**备用 A —— 并发控制（per-user 锁，⚠️ 弱化用，别主动当最难）**
...
所以最难的不是"加个锁"这个动作，而是在"事务太重、乐观锁跟外部调用冲突、悲观锁又要处理 TTL/超时/抢不到"这几个都不完美的选项里，判断出"per-user Redis 锁 + 幂等兜底 + 靠平台重投"这个组合代价最小又刚好够用。并发难的从来不是让它对，是让它在所有边界情况下都对、还不过度设计。（⚠️ 但这段别主动当"最难"讲——被追"为什么不用 mongo 条件更新/CAS"会露；只在面官已经问到并发细节时诚实带出，口径见 Q9 末尾。）

**备用 B —— 状态机设计（两个 status 字段，站得住，比并发锁更适合当第二难点）**
...
### Q9. 通知处理的演进（并发 + 乱序 + 跨交易 + 重复，一个函数里的完整故事，越全越好）

> premium 主力故事，讲"技术难点"。可直接念的口语稿，第一人称、演进式——简单实现遇到一类问题就加一层。踩坑 PR#3632 在乱序那段。跨交易那段：交易号身份判断 + 对不上时回调 Apple 裁决当前 active（按用户确认，按"已做"讲）。CAS 不主动讲、被问才答（口径见末尾）。机制代码核实过，别报没核实的影响用户数。
...
这套东西真正难的地方，不在于处理某一条事件，而在于这些平台事件是乱序、重复、延迟到达的，而且会跨多笔交易。所以核心挑战是：在这样一个不可靠的事件流上，怎么始终维持每个用户正确的状态。我下面按我当时踩坑的顺序讲，这套是怎么一步步长出来的。

最开始的实现很简单：收到通知，就读出这个用户的记录、根据通知算出新状态、写回去，先跑起来。

跑起来之后第一个撞到的是并发。同一个用户，平台可能在很短时间内连着发好几条通知，比如扣款失败紧跟着重试成功。如果两条同时进来，两个 worker 都读到同一份旧记录、各自算、各自写，后写的就把先写的覆盖了——经典的 read-modify-write 覆盖，而这是钱，覆盖错就是付费状态错。所以我加了一个针对单个用户的分布式锁，用 Redis 的 SetNX 实现，key 就是用户 id，同一个用户的通知串行处理，不同用户之间互不阻塞。

串行之后，第二个问题是乱序。就算我一条条处理，通知到达的顺序也不等于事件真正发生的顺序——平台不保证顺序，还会延迟、会重投。举个例子，一个用户先订阅、再取消、然后又重新订阅，这三条到我这的顺序完全可能是乱的，取消甚至可能比订阅先到。如果我傻按到达顺序处理，就会用一个更早发生的事件去覆盖更新的状态。所以我在每条记录上存一个"最后处理的事件时间"当水位线，新通知只有比这个时间更新才处理，旧的直接丢。这里我踩过一个坑：一开始我比的是数据库记录的更新时间，但因为处理本身有延迟，这个更新时间永远晚于事件真正发生的时间，结果反而把新通知当成旧的丢掉了。后来我专门加了个字段，存平台自己签名的那个事件时间，拿它来比，才屏蔽掉处理延迟和多机时钟的干扰。

而刚才那个重新订阅，其实还牵出更麻烦的一层，就是跨交易。用户重新订阅的时候，平台会给他一个全新的交易号——Apple 那个 originalTransactionId 一生不变，只有重新订阅才会换。所以光比时间不够了，我得先判断"这条通知说的到底是哪一笔订阅"。我是靠交易号的身份来判：交易号跟记录里存的一致，才按时间比新旧；对不上，说明是另一笔订阅，这时候只有新购买事件才允许把记录重新指向那个新交易，续费、取消这种遇到对不上的交易号就直接丢，防止一笔旧订阅的迟到通知覆盖掉当前有效的新订阅。这里还有个边界：如果取消先到、而本地记录还不存在，也就是订阅那条还没来，那这条取消没有主体可改，也直接丢——规则就是没记录的时候只有订阅事件能创建记录。

不过跨交易这种归属判断，光靠我本地的时间戳还不够可靠，因为 Apple 跨不同交易的时间戳并不保证严格单调。所以交易号对不上的时候，我会回调 Apple 的 subscription status 接口，让 Apple 直接告诉我当前哪一笔交易是 active 的，以它为准。这里 Apple 和 Google 有个有意思的差别：Google 的通知本来就只给一个 token，你必须回查它的 API 才知道状态；Apple 的通知虽然自带签名 payload，但在这种跨交易归属拿不准的场景，我同样选择回查 Apple、以平台的权威状态为准，而不是只信我手里这一条通知。

最后一个是重复。平台会重投通知，我这边一时没返回成功它就重发。串行加上刚才那个时序判断，其实已经能挡住大部分重复，因为重投那条的时间不比水位线新，会被丢掉。但我还是又做了一层显式的幂等：用平台通知自带的唯一 id——Apple 的 notificationUUID、Google 的 message id——当幂等键，处理之前先去占位。我放在 Redis，用 `SET idem:{uuid} NX EX` 带一个几天的 TTL，这条命令原子地做到"没见过才占位"，占位成功才处理，撞上了就直接跳过。放 Redis 而不是数据库，是因为一条 SetNX 就够、还能自动过期，比在数据库上建唯一索引轻；代价是 Redis 万一丢了可能漏挡一次，但状态写本身是幂等的，漏一次也没有副作用。
...
### Q10. 讲一个 project + 难点（video upload —— 可直接念的口语稿）

> video upload 主力故事，对口 LIVE Foundation Governance 组。重心压在"统一内容平台 + 状态编排 + 审核治理 + 反滥用"（Foundation+Governance），不是"用户上传体验"。下面正文是第一人称口语、可直接讲。（诚实边界：anti-abuse 是我写的可深讲；转码平台/CPP/审核团队是下游，讲接口即可，被问内部就说"那是 X 平台/团队的系统，我通过 kafka/接口跟它们交互"。）
