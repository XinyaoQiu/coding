...
**第二步，发现并发问题。** 同一个用户，平台可能在很短时间内发来好几条通知。如果它们同时进来，两个 worker 都读到同一份旧记录、各自算、各自写，后写的把先写的覆盖掉——经典的并发 read-modify-write 覆盖。所以我加了一个针对单个用户的分布式锁，用 Redis 的 SetNX 实现，key 是用户 id——同一个用户的通知串行处理，不同用户之间不阻塞。

**第三步，发现乱序问题。** 就算串行了，通知的到达顺序也不等于事件真正发生的顺序——平台不保证顺序、还会延迟、重投。举个直观的例子：一个用户先 subscribe、再 cancel、然后又 resubscribe，这三条通知到达我们这的顺序完全可能是乱的，cancel 甚至可能比 subscribe 先到。如果按到达顺序傻处理，就会用一个更早发生的事件覆盖更新的状态。所以我在每条记录上存一个"最后处理的事件时间"当水位线，新通知只有比它更新才处理，旧的丢掉。**这里踩过一个坑**：一开始我比的是数据库记录的更新时间，但因为处理有延迟，这个更新时间永远晚于事件真正发生的时间，结果反而把新的通知当成旧的丢掉了。后来专门加了个字段存**平台自己签名的事件时间**，拿这个比，而不是用我们服务器的写入时间——这样才屏蔽掉处理延迟和多机时钟的干扰。

**第四步，发现跨交易问题。** 上面那个 resubscribe 其实还牵出更麻烦的一层：用户重新订阅时，平台会给他一个**全新的交易号**（Apple 的 originalTransactionId 一生不变，重新订阅才换）。所以光比时间不够——两笔是不同的订阅，得先判"这条通知说的是哪一笔"。我们靠交易号身份来判：交易号和记录里存的一致，才按时间比新旧；不一致，说明是另一笔订阅，只有新购买（subscribe）事件才允许把记录重新指向那个新交易，其它类型（续费、取消）遇到对不上的交易号就丢——防止一笔旧订阅的迟到通知覆盖当前有效的新订阅。这里还有个边界：如果 cancel 先到、而本地记录还不存在（subscribe 还没来），那这条 cancel 没有主体可改，直接丢——规则是"没记录时只有 subscribe 能创建"。

而且跨交易这种归属判断，光靠本地记录不够可靠——因为 Apple 跨不同交易的时间戳并不保证严格单调。所以交易号对不上时，我们会**回调 Apple 的 subscription status 接口，让 Apple 直接告诉我们当前哪笔交易是 active 的，以它为准**。这一点上 Apple 和 Google 有意思的差别是：Google 的通知本来就只给一个 token、必须回查它的 API 才知道状态；Apple 的通知虽然自带签名 payload，但在跨交易归属这种拿不准的场景，我们同样选择回查 Apple、以平台的权威状态为准，而不是只信单条通知。
